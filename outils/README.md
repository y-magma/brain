# Outils Magma

> **Magma n’est pas un seul outil.** C’est un projet qui vise **beaucoup d’outils** pour **beaucoup de cas**.  
> Chaque outil a un **nom** (ex. **Weco**) — on ne dit plus « instance ».

## Trois couches (à ne pas mélanger)

| Couche | Où | Rôle |
|--------|-----|------|
| **Idées / slogans / chantiers** | [`../chantiers/`](../chantiers/) | Brainstorm Brain — matériau large |
| **Outils Magma** | **ici** (`outils/`) | Propositions / produits qui **catalysent une partie** des idées — pas toutes |
| **Exemples de questions** | [`../chantiers/banc-essai-debats/cas/`](../chantiers/banc-essai-debats/cas/) | Débats cobayes, **adaptables** à différents outils |

Un outil (Weco) peut brancher *certains* exemples et *certaines* idées.  
Un exemple (patrimoine) peut servir à *plusieurs* outils, adapté à chacun.

## Index des outils

| Outil | Catalyse quoi | Lien |
|-------|----------------|------|
| **Weco** | Comparer données / graphes d’inégalités | [`weco/`](weco/) |

Nouveau outil : copier [`_gabarit-outil.md`](_gabarit-outil.md).

## Paysage (hors Magma)

Ce qui existe déjà (Kialo, Pol.is, Wikipédia, Decidim, Stack Overflow…) :  
→ [`../chantiers/paysage-outils-existants/analyse.md`](../chantiers/paysage-outils-existants/analyse.md)

## Voir aussi

- Magma = plusieurs outils : [`../chantiers/magma-plusieurs-outils/`](../chantiers/magma-plusieurs-outils/)
- En clair : [`../workflow/en-clair.md`](../workflow/en-clair.md)
